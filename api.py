...
class SimulationParams(BaseModel):
    ...
    collapse_radius: float = 5.0
    collapse_G: float = 10.0
    slit_width: float = 2.0
    slit_separation: float = 6.0
    beam_momentum: float = 5000.0
    wall_mass: float = 1000.0
    screen_x: float = 20.0
    total_ticks: int = 5000
    run_label: str = "Custom"
    batch_size: int = 0  # 0=auto (GPU:100, CPU:1)
    num_slits: int = 2  # 1=single-slit control, 2=double-slit (default)
    wall_depth: int = 1  # Wall depth in x-layers (1=flat, >1=tunnel)
    wall_z_layers: int = 1  # Wall layers in z-axis (1=2D, >1=3D)
    ai_translation: bool = True  # Enable/disable Ollama interpretation
...
async def extract_math(params: SimulationParams):
    try:
        ...
        collider_cmd = [
            ...
            "--slit_separation", str(params.slit_separation),
            "--beam_momentum", str(params.beam_momentum),
            "--wall_mass", str(params.wall_mass),
            "--screen_x", str(params.screen_x),
            "--total_ticks", str(params.total_ticks),
            "--run_label", params.run_label,
            "--batch_size", str(params.batch_size),
            "--num_slits", str(params.num_slits),
        ...
        # Parse double-slit histogram if present
        double_slit_data = None
        if "DOUBLE_SLIT_JSON=" in collider_output:
            import json
            try:
                with open("double_slit_results.json", "r") as f:
                    double_slit_data = json.load(f)
            except Exception:
                pass
